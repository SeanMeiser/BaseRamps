...
function LightnessCurvePresets({ curve }: { curve: Curve }) {
  // Check if curve matches Linear (0,0, 1,1)
  // We use a small epsilon for float comparison just in case, though clamping usually gives exact 0/1
  const isLinear = 
    Math.abs(curve.x1 - 0) < 0.001 && 
    Math.abs(curve.y1 - 0) < 0.001 && 
    Math.abs(curve.x2 - 1) < 0.001 && 
    Math.abs(curve.y2 - 1) < 0.001;

  const label = isLinear ? "Linear" : "Custom";
...
function LightnessCurve({ curve, onChange, steps }: { curve: Curve; onChange: (c: Curve) => void; steps: number }) {
  return (
    <div className="content-stretch flex flex-col gap-[32px] items-start relative shrink-0 w-full" data-name="LightnessCurve">
      <LightnessCurvePresets curve={curve} />
...
function Swatch({ color, lValue }: { color: string; lValue: number }) {
    ...
    <div className="w-full">
      ...
      <div className="content-stretch flex gap-[8px] items-center relative shrink-0 w-full">
        {rampColors.map((colorHex, i) => {
          ...
          return (
            <Swatch key={i} color={finalColor} lValue={l} />
          );
        })}
      </div>
    </div>
